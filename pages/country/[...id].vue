    ...
    <div>
        <div class="bg-grey py-10">
            <div class="my_container">
                <div class="flex items-center gap-5">
                    ...
                    <img class="w-20 h-20 mt-2" src="../../assets/icons/arrow-right-s-black.png" alt="">
                    <nuxt-link :to="`/destinations/${data.list.continentId}`" class="underline underline-offset-2">{{ data.list && data.list.continentName ? data.list.continentName[$i18n.locale] : '' }}</nuxt-link>
                    <img class="w-20 h-20 mt-2" src="../../assets/icons/arrow-right-s-black.png" alt="">
        ...
        <div class="bg-white py-50 lg:py-80">
            <div class="container_2">
                <h1 class="title_text text-center mb-30">Top Europe trips count: {{ trips.count }}</h1>
            ...
            <Pagination  :active="Math.trunc(Number($route.query.offset)/12) + 1 || 1" :perPage="12" :items="trips.count" @change="(val: any) => changeQuery('offset', (val - 1)*12)"/>
        </div>
    </div>
</template>